...
/// \class Propagator
/// \brief Abstract base class of the library which sets up input parameter on
/// the host. Concrete implementation of calcuations is provided in derived
/// classes
/// @param FLOAT_T The floating point type to use for calculations, i.e float,
/// double
template <class FLOAT_T> class Propagator {
public:
  /// \brief Constructor
  ///
...
public:
  ...
  /// \details radii_ and rhos_ must be same size. both radii_ and rhos_ must be
  /// sorted, in the same order. The density (g/cm^3) at a distance (km) from
  /// the center of the sphere between radii_[i], exclusive, and radii_[j],
  /// inclusive, i < j  is assumed to be rhos_[j]
  /// @param radii_ List of radii
  /// @param rhos_ List of densities
  virtual void setDensity(const std::vector<FLOAT_T> &radii_,
    ...
    for (size_t i = 0; i < radii.size(); i++) {
      ...
      FLOAT_T x = -1 * sqrt(1 - (radii[i] * radii[i] /
  ...
  /// \details File must contain two columns where the first column contains the
  /// radius (km) and the second column contains the density (g/cm³). The first
  /// row must have the radius 0. The last row must have to radius of the sphere
  ///
  ...
  /// \brief Set neutrino mass differences (m_i_j)^2 in (eV)^2. no assumptions
  /// about mass hierarchy are made
  /// @param dm12sq
  /// @param dm23sq
  ...
  virtual void setEnergyList(const std::vector<FLOAT_T> &list) {
    if (list.size() != size_t(n_energies))
      throw std::runtime_error("Propagator::setEnergyList. Propagator was not "
                               "created for this number of energy nodes");
  ...
  virtual void setCosineList(const std::vector<FLOAT_T> &list) {
    if (list.size() != size_t(n_cosines))
      throw std::runtime_error("Propagator::setCosineList. Propagator was not "
                               "created for this number of cosine nodes");
  ...
  /// \details Adds a layer of length heightKM with zero density to the density
  /// model
  ...
  virtual FLOAT_T getProbability(int index_cosine, int index_energy,
                                 ProbType t) = 0;

protected:
  // for each cosine bin, determine the number of layers which will be crossed
  // by the neutrino path the atmospheric layers is excluded
  virtual void setMaxlayers() {
    for (int index_cosine = 0; index_cosine < n_cosines; index_cosine++) {
      FLOAT_T c = cosineList[index_cosine];
      const int maxLayer =
          std::count_if(coslimit.begin(), coslimit.end(),
                        [c](FLOAT_T limit) { return c < limit; });
  ...
  FLOAT_T &DM(int i, int j) { return dm[(i * 3 + j)]; }
  ...
  std::array<cudaprob3::math::ComplexNumber<FLOAT_T>, 9>
      Mix_U;                 // MNS mixing matrix

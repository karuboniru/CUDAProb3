 ...
 * __host__ __device__
 * void calculate(NeutrinoType type, const FLOAT_T* const cosinelist, int
 * n_cosines, const FLOAT_T* const energylist, int n_energies, const FLOAT_T*
 ...
 * template<typename FLOAT_T>
 * __global__
 * void calculateKernel(NeutrinoType type, const FLOAT_T* const cosinelist, int
 * n_cosines, const FLOAT_T* const energylist, int n_energies, const FLOAT_T*
 * const radii, const FLOAT_T* const rhos, const int* const maxlayers, FLOAT_T
 * ProductionHeightinCentimeter, FLOAT_T* const result)
 *
...
// in device code, we need to access the device global constants instead of host
// global constants
#ifdef __CUDA_ARCH__

#define U(i, j) ((math::ComplexNumber<FLOAT_T> *)mix_data_device)[(i * 3 + j)]
#define DM(i, j) ((FLOAT_T *)mass_data_device)[(i * 3 + j)]
#define AXFAC(a, b, c, d, e)                                                   \
  ((FLOAT_T *)A_X_factor_device)[a * 3 * 3 * 3 * 4 + b * 3 * 3 * 4 +           \
                                 c * 3 * 4 + d * 4 + e]
...
#define AXFAC(a, b, c, d, e)                                                   \
  ((FLOAT_T *)                                                                 \
       A_X_factor)[a * 3 * 3 * 3 * 4 + b * 3 * 3 * 4 + c * 3 * 4 + d * 4 + e]
...
__constant__ double
    A_X_factor_device[81 * 4]; // precomputed factors which only depend on the
                               // mixing matrix for faster calculation
 ...
 */
template <typename FLOAT_T> void setMixMatrix(math::ComplexNumber<FLOAT_T> *U) {
  ...
  cudaMemcpyToSymbol(mix_data_device, U,
                     sizeof(math::ComplexNumber<FLOAT_T>) * 9, 0, H2D);
  CUERR;
  cudaMemcpyToSymbol(A_X_factor_device, A_X_factor, sizeof(FLOAT_T) * 81 * 4, 0,
                     H2D);
  CUERR;
#endif
}
...
template <typename FLOAT_T> void setMassDifferences(FLOAT_T *dm) {
  memcpy((FLOAT_T *)mass_data, dm, sizeof(FLOAT_T) * 9);
#ifdef __CUDA__
  cudaMemcpyToSymbol(mass_data_device, dm, sizeof(FLOAT_T) * 9, 0,
...
//
template <typename FLOAT_T> void prepare_getMfast(NeutrinoType type) {
  ...
  /* Equation (21) */
  argV =
      (2.0 * alphaV * alphaV * alphaV - 9.0 * alphaV * betaV + 27.0 * gammaV) /
...
#ifdef __CUDA__
  cudaMemcpyToSymbol(mass_order_device, order, sizeof(int) * 3, 0,
                     cudaMemcpyHostToDevice);
  CUERR;
#endif
}
 ...
 * and d_dmMatVac, which is the mass difference matrix between induced masses
 * and vacuum masses
 *
 * The strategy to sort out the three roots is to compute the vacuum
 * mass the same way as the "matter" masses are computed then to sort
 * the results according to the input vacuum masses. Subsequently, the "matter"
 * masses are calculated, using the found sorting for vacuum masses
 *
 * In the original implementation the order of vacuum masses is computed for
 * each bin. However, the ordering of vacuum masses does only depend on the
 * constant neutrino mixing matrix. Thus, the ordering can be precomputed, which
...
getMfast(const FLOAT_T Enu, const FLOAT_T rho, const NeutrinoType type,
  ...
  const FLOAT_T beta =
      DM(0, 1) * DM(0, 2) + fac * (DM(0, 1) * (1.0 - U(0, 1).re * U(0, 1).re -
                                               U(0, 1).im * U(0, 1).im) +
                                   DM(0, 2) * (1.0 - U(0, 2).re * U(0, 2).re -
                                               U(0, 2).im * U(0, 2).im));

  const FLOAT_T gamma = fac * DM(0, 1) * DM(0, 2) *
                        (U(0, 0).re * U(0, 0).re + U(0, 0).im * U(0, 0).im);

  /* Compute the argument of the arc-cosine */
  const FLOAT_T tmp =
      alpha * alpha - 3.0 * beta < 0 ? 0 : alpha * alpha - 3.0 * beta;

  /* Equation (21) */
  const FLOAT_T argtmp =
      (2.0 * alpha * alpha * alpha - 9.0 * alpha * beta + 27.0 * gamma) /
...
get_product(const FLOAT_T L, const FLOAT_T E, const FLOAT_T rho,
            ...
            math::ComplexNumber<FLOAT_T> product[][3][3]) {
...
#pragma unroll
    ...
    for (int m = 0; m < 3; m++) {
      ...
      twoEHmM[n][m][0].im =
          -fac * (U(0, n).re * U(0, m).im - U(0, n).im * U(0, m).re);
      twoEHmM[n][m][1].re =
          -fac * (U(0, n).re * U(0, m).re + U(0, n).im * U(0, m).im);
      twoEHmM[n][m][1].im =
          -fac * (U(0, n).re * U(0, m).im - U(0, n).im * U(0, m).re);
...
#pragma unroll
        ...
        for (int j = 0; j < 3; j++) {
          ...
          A[n][m].im += AXFAC(n, m, i, j, 2) * X[i][j].im +
...
/*
 * Get 3x3 transition amplitude Aout for neutrino with energy E travelling Len
...
HOSTDEVICEQUALIFIER FLOAT_T getTraversedDistanceOfLayer(
  ...
  const FLOAT_T CrossThis =
      2.0 * sqrt(radii[i] * radii[i] - (Constants<FLOAT_T>::REarth()) *
                                           (Constants<FLOAT_T>::REarth()) *
                                           (1 - cosine_zenith * cosine_zenith));
  const FLOAT_T CrossNext =
      2.0 * sqrt(radii[i + 1] * radii[i + 1] -
                 (Constants<FLOAT_T>::REarth()) *
                     ((FLOAT_T)Constants<FLOAT_T>::REarth()) *
                     (1 - cosine_zenith * cosine_zenith));
...
#pragma omp parallel for schedule(dynamic)
  for (int index_cosine = 0; index_cosine < n_cosines; index_cosine += 1) {
    ...
    const FLOAT_T PathLength =
        sqrt((Constants<FLOAT_T>::REarthcm() + ProductionHeightinCentimeter) *
                 (Constants<FLOAT_T>::REarthcm() +
                  ProductionHeightinCentimeter) -
             (Constants<FLOAT_T>::REarthcm() * Constants<FLOAT_T>::REarthcm()) *
                 (1 - cosine_zenith * cosine_zenith)) -
        Constants<FLOAT_T>::REarthcm() * cosine_zenith;

    const FLOAT_T TotalEarthLength =
        -2.0 * cosine_zenith * Constants<FLOAT_T>::REarthcm(); // in [cm]
...
#pragma unroll
      ...
      for (int i = 0; i <= MaxLayer; i++) {
        const FLOAT_T distance = getTraversedDistanceOfLayer(
            radii, i, MaxLayer, PathLength, TotalEarthLength, cosine_zenith);
        ...
        } else if (i < MaxLayer) { // not the innermost layer, can reuse current
                                   // TransitionMatrix
          clear_complex_matrix(TransitionTemp);
          multiply_complex_matrix(TransitionMatrix, finalTransitionMatrix,
                                  TransitionTemp);
          copy_complex_matrix(TransitionTemp, finalTransitionMatrix);

          clear_complex_matrix(TransitionTemp);
          multiply_complex_matrix(TransitionMatrixCoreToMantle,
                                  TransitionMatrix, TransitionTemp);
      ...
      clear_complex_matrix(TransitionTemp);
      multiply_complex_matrix(TransitionMatrixCoreToMantle,
                              finalTransitionMatrix, TransitionTemp);
      copy_complex_matrix(TransitionTemp, finalTransitionMatrix);
...
#pragma unroll
        ...
        for (int outflv = 0; outflv < 3; outflv++) {
          ...
          result[resultIndex + (unsigned long long)(n_energies) *
...
#ifdef __CUDA__
template <typename FLOAT_T>
KERNEL __launch_bounds__(64, 8) void calculateKernel(
...
template <typename FLOAT_T>
void callCalculateKernelAsync(
    dim3 grid, dim3 block, cudaStream_t stream, NeutrinoType type,
